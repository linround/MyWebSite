...
function ListItemTemplate({ text, translateValue, show, })  {
  return (
    <li className="dropdown-list-item" style={{
      opacity: show ? 1 : 0,
    }}>
      <button className="dropdown-button list-button" data-translate-value={translateValue}>
...
export default class DropDownOptions extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      showOptions: false,
      dropdownListContainerMaxHeight: 0,
      targetText: '',
      clickedItemText: 'facebook',
      listItems: ['Linkedin', 'Instagram', 'Facebook', 'Twitter', 'Youtube'],
    }
    this.onMainButtonClick = this.onMainButtonClick.bind(this)
    this.onDropdownListMousemove = this.onDropdownListMousemove.bind(this)
    this.onDropdownListClick = this.onDropdownListClick.bind(this)
    this.onDropdownListMouseover = this.onDropdownListMouseover.bind(this)
  }
  componentDidMount() {

  }
  onDropdownListMouseover(e) {
    const root = document.documentElement
    const translateValue = e.target.dataset.translateValue
    root.style.setProperty('--translate-value', `${translateValue * 0.73}px`)
  }
  onDropdownListClick(e) {
    const clickedItemText = e.target.innerText.toLowerCase()
      .trim()
    this.setState({
      clickedItemText,
    })
  ...
  onDropdownListMousemove(e) {
    const root = document.documentElement
    const dropdownList = e.currentTarget
  ...
  onMainButtonClick() {
    ...
    this.setState({
      ...
      dropdownListContainerMaxHeight: !showOptions ? dropdownOpenHeight : 0,
    })
  }
  render() {
    const listItems = this.state?.listItems || []
    const targetText = this.state.targetText
    const icons = this.state.icons
    const IconHtml = this.iconTemplate(icons[targetText])
    const clickedItemText = this.state.clickedItemText
    const dropdownTitleIconHtml = this.iconTemplate(icons[clickedItemText])
    const showOptions = this.state.showOptions
    const dropdownListContainerMaxHeight = this.state.dropdownListContainerMaxHeight
    return (
      <>
        <div className={`${styles.dropdownContainer}`}>
          <button className="dropdown-button main-button" onClick={this.onMainButtonClick}>
            <span className="dropdown-title-icon">
              {dropdownTitleIconHtml}
            </span>
            <span className="dropdown-title text-truncate">{ clickedItemText }</span>
          ...
          <div className="dropdown-list-container" style={{
            maxHeight: dropdownListContainerMaxHeight + 'rem',
          }}>
            <div className="dropdown-list-wrapper">
              <ul
                className="dropdown-list"
                onClick={this.onDropdownListClick}
                onMouseOver={this.onDropdownListMouseover}
                onMouseMove={this.onDropdownListMousemove}>
                {
                  listItems.map((text, index) => (
                    <ListItemTemplate key={index} text={text} translateValue={100 * index} show={showOptions}/>
                  ))
                }
              </ul>
