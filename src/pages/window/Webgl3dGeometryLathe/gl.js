...
export function myRender(canvas) {
  ...
  function generateMesh(bufferInfo) {
    ...
    const points = simplifyPoints(
      tempPoints, 0, tempPoints.length, data.distance
    )
    const arrays = lathePoints(
      points, data.startAngle, data.endAngle, data.divisions, data.capStart, data.capEnd
    )
    ...
    if (!bufferInfo) {
      ...
      gl.bufferData(
        gl.ARRAY_BUFFER, new Float32Array(arrays.position), gl.STATIC_DRAW
      )
      gl.bindBuffer(gl.ARRAY_BUFFER, bufferInfo.attribs.a_texcoord.buffer)
      gl.bufferData(
        gl.ARRAY_BUFFER, new Float32Array(arrays.texcoord), gl.STATIC_DRAW
      )
      gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, bufferInfo.indices)
      gl.bufferData(
        gl.ELEMENT_ARRAY_BUFFER, new Uint16Array(arrays.indices), gl.STATIC_DRAW
      )
...
//   gl_FragColor = vec4(v_texcoord, 0, 1);
  ...
  `
  const programInfo = webglUtils.createProgramInfo(gl, [v, f])

  const texInfo = loadImageAndCreateTextureInfo(imgTexture, render)
  ...
  function render(time) {
    ...
    webglUtils.drawBufferInfo(
      gl, bufferInfo, data.triangles ? gl.TRIANGLE : gl.LINES
    )
  }
  ...
  function getPointsOnBezierCurve(
    points, offset, numPoints
  ) {
    const cpoints = []
    for (let i = 0; i < numPoints; ++i) {
      const t = i / (numPoints - 1)
      cpoints.push(getPointOnBezierCurve(
        points, offset, t
      ))
    }
    return cpoints
  ...
  function getPointsOnBezierCurveWithSplitting(
    points, offset, tolerance, newPoints
  ) {
    ...
    if (flatness(points, offset) < tolerance) {
      ...
      getPointsOnBezierCurveWithSplitting(
        [p1, q1, r1, red], 0, tolerance, outPoints
      )
      // do 2nd half
      getPointsOnBezierCurveWithSplitting(
        [red, r2, q3, p4], 0, tolerance, outPoints
      )
  ...
  function simplifyPoints(
    points, start, end, epsilon, newPoints
  ) {
    const outPoints = newPoints || []
    ...
    for (let i = start + 1; i < end - 1; ++i) {
      const distSq = v2.distanceToSegmentSq(
        points[i], s, e
      )
      if (distSq > maxDistSq) {
        maxDistSq = distSq
    ...
    if (Math.sqrt(maxDistSq) > epsilon) {
      ...
      // split
      simplifyPoints(
        points, start, maxNdx + 1, epsilon, outPoints
      )
      simplifyPoints(
        points, maxNdx, end, epsilon, outPoints
      )
  ...
  function lathePoints(
    ...
    for (let division = 0; division < numDivisions; ++division) {
      ...
      for (let quad = 0; quad < quadsDown; ++quad) {
        indices.push(
          column1Offset + quad, column1Offset + quad + 1, column2Offset + quad
        )
        indices.push(
          column1Offset + quad + 1, column2Offset + quad + 1, column2Offset + quad
        )
      }
    }
  ...
  //   document.querySelector('#ui'), data, [
...
const v2 = (function() {
  ...
  function distanceToSegment(
    ...
    return Math.sqrt(distanceToSegmentSq(
      p, v, w
    ))
  }
